...
gom theo 9 nhóm tính năng, cộng thêm 1 file JS mới (`14-admin-force-end-match.js`) được vá thêm sau đó
để bổ sung tính năng cưỡng chế kết thúc trận — tổng hiện tại là 45 file: 1 HTML + 1 CSS + 43 JS.
Load game **chỉ cần mở `index.html`**, các file còn lại được nó tự gọi qua thẻ `<script>`.
...
Riêng file mới `14-admin-force-end-match.js` (tính năng cưỡng chế kết thúc trận, thêm sau — xem bảng
dưới) thì **phải** là thẻ `<script>` cuối cùng trong `index.html`, sau cả
`multiplayer/05-toast-modal-and-turn-config.js`, vì nó bọc thêm (không ghi đè) `changeScreen` và
`endGame` của các phase trước — bọc sớm hơn sẽ bị các phase sau đè mất.
...
| **Admin cưỡng chế kết thúc trận sớm** (nút chỉ hiện khi đã nhập đúng PIN, nằm ngay dưới BXH realtime; gửi lệnh `FORCE_END` cho toàn bộ client) | `14-admin-force-end-match.js` (file mới, đứng riêng ở gốc repo, không thuộc nhóm nào trong 9 nhóm cũ) |
...
Đã verify: ráp lại toàn bộ 44 file gốc (trước khi thêm `14-admin-force-end-match.js`) cho ra đúng
100% nội dung file gốc (diff bằng tool, không lệch dòng code nào, chỉ lệch vài dòng trắng vô hại ở
ranh giới file). Từng file `.js`, kể cả file mới thêm sau, đã chạy `node --check` qua, không lỗi cú pháp.
